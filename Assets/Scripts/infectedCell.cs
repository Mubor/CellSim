...
public class infectedCell : MonoBehaviour
{
    ...
    void DeathOfCell(GameObject thisObject, int infLvl, bool zoom)
    {
        ...
        for (int i = 0; i < Random.Range(5, 9); i++)
        {
            ...
            newObject.AddComponent<Virus>().SetParams(newObject, newObject.tag, newObject.name, Random.Range(80, 101), true, 0);
        }
    }
    ...
    void AddPhysic(GameObject obj, bool isCapsule)
    {
        if (isCapsule)
        {
            obj.AddComponent<Motion>().Params(Speed, DirectionChange, false, direction);
        }
        else
        {
            obj.AddComponent<MotionSphere>().Params(Speed, DirectionChange, false, direction);
        }
    }
    ...
    void DetailedPrefabs()
    {
        ...
        {
            //print("I'm " + this.infectCell.name + " and my infectionLVL is: " + InfectLvl);
    ...
    public void setParams(GameObject gameObject, string tag, string name, float localScale, int speed, int dir, int infLvl, int direction)
    {
        Speed = speed;
